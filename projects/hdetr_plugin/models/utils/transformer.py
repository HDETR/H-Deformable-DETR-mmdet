...
def nchw_to_nlc(x):
    ...
    Returns:
        Tensor: The output tensor of shape [N, L, C] after caponversion.
    """
...
@TRANSFORMER_LAYER.register_module(name='DetrTransformerDecoderLayer', force=True)
...
@TRANSFORMER_LAYER.register_module(name='DETRBaseTransformerLayer', force=True)
class DETRBaseTransformerLayer(BaseTransformerLayer):
    """Implements decoder layer in DETR transformer.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(DETRBaseTransformerLayer, self).__init__(
            attn_cfgs=attn_cfgs,
            feedforward_channels=feedforward_channels,
            ffn_dropout=ffn_dropout,
            operation_order=operation_order,
...
@TRANSFORMER_LAYER_SEQUENCE.register_module(name='DetrTransformerEncoder', force=True)
class DetrTransformerEncoder(TransformerLayerSequence):
    ...
    def __init__(
        self, *args, post_norm_cfg=dict(type="LN"), use_checkpoint=False, **kwargs
    ):
...
@TRANSFORMER_LAYER_SEQUENCE.register_module(name='DetrTransformerDecoder', force=True)
...
@TRANSFORMER.register_module(name='Transformer', force=True)
...
@TRANSFORMER_LAYER_SEQUENCE.register_module(name='DeformableDetrTransformerDecoder', force=True)
...
@TRANSFORMER.register_module(name='DeformableDetrTransformer', force=True)
class DeformableDetrTransformer(Transformer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if custom:
            super(Transformer, self).__init__(init_cfg=init_cfg)
            self.decoder = build_transformer_decoder(decoder)
            self.encoder = build_transformer_encoder(encoder)
        else:
            super(DeformableDetrTransformer, self).__init__(
                init_cfg=init_cfg, decoder=decoder, encoder=encoder
            )
        ...
        **kwargs,
        ...
        if self.custom:
            inter_states, inter_references = self.decoder(
                ...
                self_attn_mask=decoder_self_attn_mask[0]
                if decoder_self_attn_mask
                else None,
            )
            memory = memory.permute(1, 0, 2)
        else:
        ...
        if self.as_two_stage:
            if self.return_encoder:
                return (
                    ...
                    enc_outputs_class,
                    enc_outputs_coord_unact,
                    memory.permute(1, 0, 2)[:, : level_start_index[1]].reshape(
...
@TRANSFORMER.register_module(name='DynamicConv', force=True)
